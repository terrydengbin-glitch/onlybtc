...
import {
  ...
  type P45ArticleHistory,
  type P45Articles,
  type P45Dashboard,
  type P45Evidence,
  type P45Invalidation,
  type P45Overview,
  type P45RunJob,
  type P45RadarModules,
  type P45Runs,
  type RadarRuntimePayload,
} from './api'
...
type DashboardStore = {
  dashboard: P45Dashboard | null
  overview: P45Overview | null
  radarModulesPayload: P45RadarModules | null
  articles: P45Articles | null
  articleHistory: P45ArticleHistory | null
  invalidation: P45Invalidation | null
  runs: P45Runs | null
  auditReports: AuditReports | null
  dataQuality: Record<string, unknown> | null
  evidence: Record<string, unknown> | null
  analysts: Record<string, unknown> | null
  llm: Record<string, unknown> | null
  settings: Record<string, unknown> | null
  alerts: Record<string, unknown> | null
  events: Record<string, unknown> | null
  eventWindow: EventWindowPayload | null
  eventWindowTimeline: EventWindowPayload | null
  eventWindowCalendar: EventWindowPayload | null
  eventWindowAlerts: EventWindowPayload | null
  eventWindowDaemon: EventWindowPayload | null
  eventWindowSources: EventWindowPayload | null
  eventWindowSourceFetches: EventWindowPayload | null
  eventWindowRunOnceResult: EventWindowPayload | null
  eventWindowAuditBundle: EventWindowPayload | null
  radarRuntimeDaemon: RadarRuntimePayload | null
  radarRuntimeCockpit: RadarRuntimePayload | null
  radarRuntimeModules: RadarRuntimePayload | null
  radarRuntimeRunOnceResult: RadarRuntimePayload | null
  selectedRadarDetail: JsonRecord | null
  selectedEvidenceDetail: JsonRecord | null
  selectedSourceDetail: JsonRecord | null
  selectedSourceAuthState: JsonRecord | null
  selectedSourceLastCapture: JsonRecord | null
  selectedSourceActionResult: JsonRecord | null
  selectedHistory: JsonRecord | null
  selectedRun: JsonRecord | null
  activeRunJob: P45RunJob | null
  activeRunJobId: string
  eventStreamStatus: string
  eventStreamLastEvent: JsonRecord | null
  llmRunEnabled: boolean
  routeContext: {
    final_run_id: string
    pack_id: string
    module_id: string
...
let runJobPollTimer: ReturnType<typeof window.setTimeout> | null = null
let refreshLatestAfterJob: (() => Promise<void>) | null = null
let lastDecisionReadyRefreshJobId = ''
let deferredLatestLoadTimer: ReturnType<typeof window.setTimeout> | null = null
let eventSource: EventSource | null = null

const state = reactive<DashboardStore>({
  dashboard: null,
  overview: null,
...
function trackError(err: unknown) {
...
function rememberEndpointError(err: unknown) {
  if (err instanceof ApiClientError) {
    const duplicate = state.errors.some(
      (item) =>
        item.endpoint === err.context.endpoint &&
...
function forgetEndpointError(endpoint: string, method = 'GET') {
  state.errors = state.errors.filter((item) => item.endpoint !== endpoint || item.method !== method)
}

function listFromPayload(payload: P45RadarModules | null, fallback: JsonRecord[] | undefined) {
  return payload?.modules ?? payload?.radar_modules ?? fallback ?? []
}

function parseEvidenceScope(evidenceId: string, modules: JsonRecord[]) {
...
function syncRunContext() {
  const lineage = (state.dashboard?.run_lineage ?? state.overview?.run_lineage ?? state.runs?.latest ?? {}) as JsonRecord
...
function isTerminalRunJob(job: P45RunJob | null) {
  const status = String(job?.status ?? '').toLowerCase()
  return ['completed', 'completed_with_llm_errors', 'failed', 'cancelled'].includes(status)
}

function assignRunJob(job: P45RunJob | null) {
  ...
  const lineage = ((job?.run_lineage as JsonRecord | undefined) ?? {}) as JsonRecord
  if (job) {
    state.runs = {
      ...job,
      latest: lineage,
      stages: (job.stages as JsonRecord[] | undefined) ?? [],
    } as P45Runs
  }
...
function scheduleRunJobPoll(jobRunId: string) {
  if (runJobPollTimer) window.clearTimeout(runJobPollTimer)
  runJobPollTimer = window.setTimeout(() => {
    void pollRunJob(jobRunId)
  }, 2500)
}

async function pollRunJob(jobRunId: string) {
...
function startEventStream() {
  if (state.routeContext.isHistorical) return
...
function assignLatestPayload(key: LatestLoadKey, value: unknown) {
  switch (key) {
    case 'dashboard':
      state.dashboard = value as P45Dashboard
      break
    case 'overview':
      state.overview = value as P45Overview
      break
    case 'radarModulesPayload':
      state.radarModulesPayload = value as P45RadarModules
      break
    case 'articles':
      state.articles = value as P45Articles
      break
    case 'articleHistory':
      state.articleHistory = value as P45ArticleHistory
      break
    case 'invalidation':
      state.invalidation = value as P45Invalidation
      break
    case 'runs':
      state.runs = value as P45Runs
      break
    case 'auditReports':
      state.auditReports = value as AuditReports
      break
    case 'dataQuality':
      state.dataQuality = value as JsonRecord
      break
    case 'alerts':
      state.alerts = value as JsonRecord
      break
    case 'events':
      state.events = value as JsonRecord
      break
    case 'eventWindow':
      state.eventWindow = value as EventWindowPayload
      break
    case 'eventWindowTimeline':
      state.eventWindowTimeline = value as EventWindowPayload
      break
    case 'eventWindowCalendar':
      state.eventWindowCalendar = value as EventWindowPayload
      break
    case 'eventWindowAlerts':
      state.eventWindowAlerts = value as EventWindowPayload
      break
    case 'eventWindowDaemon':
      state.eventWindowDaemon = value as EventWindowPayload
      break
    case 'eventWindowSources':
      state.eventWindowSources = value as EventWindowPayload
      break
    case 'eventWindowSourceFetches':
      state.eventWindowSourceFetches = value as EventWindowPayload
      break
    case 'radarRuntimeDaemon':
      state.radarRuntimeDaemon = value as RadarRuntimePayload
      break
    case 'radarRuntimeCockpit':
      state.radarRuntimeCockpit = value as RadarRuntimePayload
      break
    case 'radarRuntimeModules':
      state.radarRuntimeModules = value as RadarRuntimePayload
      break
    case 'settings':
      state.settings = value as JsonRecord
      break
    case 'evidence':
      state.evidence = value as P45Evidence
      break
    case 'analysts':
      state.analysts = value as JsonRecord
      break
    case 'llm':
      state.llm = value as JsonRecord
      break
  }
}
...
export function useOnlybtcStore() {
  ...
  async function refreshLatest() {
    ...
    try {
      ...
      const deferredLoaders: LatestLoader[] = [
      ...
      const runLoader = async (loader: LatestLoader) => {
        try {
          assignLatestPayload(loader.key, await loader.request())
          if (loader.endpoint) forgetEndpointError(loader.endpoint, loader.method)
          syncRunContext()
          ...
          if (loader.critical) criticalFailures.push(message)
        }
      }

      const dashboardLoader = loaders.find((loader) => loader.key === 'dashboard')
      if (dashboardLoader) await runLoader(dashboardLoader)
      if (!state.dashboard) {
        const overviewLoader = loaders.find((loader) => loader.key === 'overview')
        if (overviewLoader) await runLoader(overviewLoader)
      }
      ...
      if (deferredLatestLoadTimer) window.clearTimeout(deferredLatestLoadTimer)
      deferredLatestLoadTimer = window.setTimeout(() => {
        void Promise.allSettled(
          deferredLoaders.map(async (loader) => {
            try {
              assignLatestPayload(loader.key, await loader.request())
              if (loader.endpoint) forgetEndpointError(loader.endpoint, loader.method)
            } catch (err) {
  ...
  function setLlmRunEnabled(value: boolean) {
    state.llmRunEnabled = value
    window.localStorage.setItem(LLM_RUN_ENABLED_KEY, value ? 'true' : 'false')
  }

  async function runFullChain(options: { llmEnabled?: boolean } = {}) {
    state.running = true
    state.error = null
    try {
      ...
      assignRunJob(job)
      const jobRunId = String(job.job_run_id ?? job.run_id ?? '')
      if (jobRunId) scheduleRunJobPoll(jobRunId)
    } catch (err) {
      trackError(err)
      state.running = false
      window.localStorage.removeItem(ACTIVE_RUN_JOB_KEY)
    } finally {
      if (!state.activeRunJobId) state.running = false
    }
  }
  ...
  async function resumeActiveRunJob() {
    ...
    try {
      ...
      if (!job || job.status === 'missing') return null
      assignRunJob(job)
      const jobRunId = String(job.job_run_id ?? job.run_id ?? '')
      if (jobRunId && !isTerminalRunJob(job)) scheduleRunJobPoll(jobRunId)
      return job
    } catch (err) {
      rememberEndpointError(err)
      window.localStorage.removeItem(ACTIVE_RUN_JOB_KEY)
  ...
  async function loadRadarDetail(moduleId: string) {
  ...
  async function loadEvidenceDetail(evidenceId: string) {
    ...
    try {
      ...
      const resolved = (state.selectedEvidenceDetail?.resolution as JsonRecord | undefined)
      ...
      if (err instanceof ApiClientError && err.context.status === 404) {
        ...
        if (scope) {
          const cached = ((state.evidence?.items as JsonRecord[] | undefined) ?? []).find(
            (item) =>
              String(item.radar_module ?? item.module_id) === scope.module_id &&
          ...
          const item = cached ?? ((payload as P45Evidence | null)?.items?.[0] as JsonRecord | undefined)
          if (item?.evidence_id) {
            state.routeContext.evidence_id = String(item.evidence_id)
            state.selectedEvidenceDetail = {
              run_lineage: (payload as JsonRecord | null)?.run_lineage ?? state.dashboard?.run_lineage ?? {},
              evidence: item,
  ...
  async function openSourceVerifyWindow(sourceId: string) {
  ...
  async function retrySourceCollect(sourceId: string) {
  ...
  async function loadSourceLastCapture(sourceId: string) {
  ...
  async function loadHistory(finalRunId: string) {
  ...
  async function pauseEventWindowDaemon() {
    ...
    if (result) state.eventWindowDaemon = result
    return result
  }
  ...
  async function refreshEventWindowLatest() {
    if (state.routeContext.isHistorical) return null
    const loaders: LatestLoader[] = [
      { key: 'eventWindow', request: api.getEventWindowLatest, endpoint: '/api/event-window/latest' },
      { key: 'eventWindowTimeline', request: () => api.getEventWindowTimeline(100), endpoint: '/api/event-window/timeline?limit=100' },
      { key: 'eventWindowCalendar', request: () => api.getEventWindowCalendar(30), endpoint: '/api/event-window/calendar?limit=30' },
  ...
  async function runEventWindowOnce() {
    ...
    if (result) {
      ...
      state.eventWindow = {
        status: 'ok',
        event_window: result.event_window as JsonRecord,
      } as EventWindowPayload
      state.eventWindowDaemon = {
        status: 'ok',
        daemon: result.daemon as JsonRecord,
      } as EventWindowPayload
      await refreshLatest()
    }
    return result
  ...
  async function pauseRadarRuntimeDaemon() {
    ...
    if (result) state.radarRuntimeDaemon = result
    return result
  }
